...
def smoothen(data_tensor, kernel_size = 5):
    data_array = data_tensor.numpy()
    sample_sz, seq_length, num_columns = data_array.shape
    smoothed_data = np.zeros_like(data_array)
    for col in range(num_columns):
        for sample in range(sample_sz):
            smoothed_data[sample, :, col] = medfilt(data_array[sample, :, col], kernel_size)
    smoothed_tensor = torch.from_numpy(smoothed_data)
...
if __name__ == "__main__":
    ...
    try:
        ...
        with open("StockPredictionDataset.pkl", "wb") as fw:
            pickle.dump(data, fw)
    data.data_30 = smoothen(data.data_30)
    data.data_daily = smoothen(data.data_daily)
